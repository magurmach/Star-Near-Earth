...
/*
no class called FHsparseMat.h provided and provided codes on assignments on
Sparse Matrix doesn't compile. So, no choice was left without using our hand
made matrix in stead of sparse matrix and no standard defition of FHsparseMat
was provided so we can code that again.
*/
   ...
   */
    ...
    */
    ...
    for(i=0;i<arraySize;i++)
    {
        ...
        if(mat.getMark(intX,intY)==' ')
        {
            mat.setMark(intX,intY,mark);
        }
        else
        {
            int i;
            ...
            for(i=0;i<4;i++)
            {
                if(intX+dx[i]>=0 && intX+dx[i]<35 && intY+dy[i]>=0 && intY+dy[i]<70)
                {
                    if(mat.getMark(intX+dx[i],intY+dy[i])==' ')
                    {
                        mat.setMark(intX+dx[i],intY+dy[i],mark);
                    }
                }
            }
        }
    ...
    for(i=0;i<arraySize;i++)
    {
        ...
        if(mat.getMark(intX,intZ)==' ')
        {
            mat.setMark(intX,intZ,mark);
        }
        else
        {
            int i;
            ...
            for(i=0;i<4;i++)
            {
                if(intX+dx[i]>=0 && intX+dx[i]<35 && intZ+dz[i]>=0 && intZ+dz[i]<70)
                {
                    if(mat.getMark(intX+dx[i],intZ+dz[i])==' ')
                    {
                        mat.setMark(intX+dx[i],intZ+dz[i],mark);
                    }
                }
            }
        }
    ...
    for(i=0;i<arraySize;i++)
    {
        ...
        if(mat.getMark(intY,intZ)==' ')
        {
            mat.setMark(intY,intZ,mark);
        }
        else
        {
            int i;
            int dz[]={0,0,1,-1},dy[]={1,-1,0,0};
            for(i=0;i<4;i++)
            {
                if(intZ+dz[i]>=0 && intZ+dz[i]<70 && intY+dy[i]>=0 && intY+dy[i]<35)
                {
                    if(mat.getMark(intY+dy[i],intZ+dz[i])==' ')
                    {
                        mat.setMark(intY+dy[i],intZ+dz[i],mark);
                    }
                }
            }
        }
